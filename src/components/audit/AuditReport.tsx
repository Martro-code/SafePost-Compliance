...
async function generateHtml(session: AuditSession, practiceName: string): Promise<string> {
  ...
  // ── HTML template with {{PLACEHOLDER}} markers ────────────────────────────
...
<body>
  ...
  <div class="page">
    ...
    <div class="report-practice-name">{{PRACTICE_NAME}}</div>
    <div class="report-date">Audit completed: {{AUDIT_DATE}}</div>
    ...
    <div class="score-card">
      ...
      <div class="score-grid">
        <div class="score-stat">
          <div class="score-stat-value">{{PAGES_ANALYSED}}</div>
          <div class="score-stat-label">Pages Analysed</div>
        </div>
        <div class="score-stat">
          <div class="score-stat-value">{{COMPLIANT_COUNT}}</div>
          <div class="score-stat-label">Compliant</div>
        </div>
        <div class="score-stat">
          <div class="score-stat-value">{{WARNINGS_COUNT}}</div>
          <div class="score-stat-label">Warnings</div>
        </div>
        <div class="score-stat">
          <div class="score-stat-value">{{ISSUES_COUNT}}</div>
  ...
  const htmlContent = template
    .replace(/\{\{PRACTICE_NAME\}\}/g, escHtml(practiceName || 'Your Practice'))
    .replace(/\{\{AUDIT_DATE\}\}/g, auditDate)
    .replace(/\{\{SCORE\}\}/g, String(score))
    .replace(/\{\{SCORE_COLOR\}\}/g, scoreColor)
    .replace(/\{\{SCORE_INTERPRETATION\}\}/g, scoreInterpretation)
    .replace(/\{\{PAGES_ANALYSED\}\}/g, String(analysedSteps.length))
    .replace(/\{\{COMPLIANT_COUNT\}\}/g, String(passCount))
    .replace(/\{\{WARNINGS_COUNT\}\}/g, String(warnCount))
    .replace(/\{\{ISSUES_COUNT\}\}/g, String(failCount))
    .replace(/\{\{PAGE_BLOCKS\}\}/g, pageBlocksHtml);
