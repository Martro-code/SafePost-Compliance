...
const Settings: React.FC = () => {
  ...
  // Toggle state
  const notificationPrefs = JSON.parse(localStorage.getItem('safepost_notification_prefs') || '{}');
  const inAppNotifications = notificationPrefs.complianceResults !== false || notificationPrefs.guidelineUpdates !== false || notificationPrefs.billingActivity !== false || notificationPrefs.newFeatures !== false;
  const twoFactor = sessionStorage.getItem('safepost_2fa') === 'true';
  const emailPrefs = JSON.parse(localStorage.getItem('safepost_email_prefs') || '{}');
  const emailUpdates = emailPrefs.productUpdates !== false || emailPrefs.complianceAlerts !== false || emailPrefs.usageSummaries !== false;
          ...
          <div className="bg-white rounded-2xl border border-black/[0.06] shadow-sm dark:bg-gray-800 dark:border-gray-700">
            ...
            <div className="flex items-center justify-between p-6 md:px-8">
              ...
              <button
                ...
                className={`relative w-11 h-6 rounded-full transition-colors duration-200 ${inAppNotifications ? 'bg-blue-600' : 'bg-gray-200 dark:bg-gray-600'}`}
              >
                <span className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow-sm transition-transform duration-200 ${inAppNotifications ? 'translate-x-5' : 'translate-x-0'}`} />
              </button>
            </div>
